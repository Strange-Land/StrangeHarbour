...
namespace BoatAttack
{
    ...
    public class WakeGenerator : MonoBehaviour
    {
        ...
        public GameObject wakePrefab; // the wake prefab, preset linerenderer
        ...
        private IEnumerator GenerateWake(Wake wake)
        {
            for (var i = 0; i < 2; i++)
            {
                ...
                var wakeLoading = InstantiateAsync<GameObject>(wakePrefab, Vector3.zero, Quaternion.Euler(90f, 0, 0));
                yield return wakeLoading;
               
                _lrs.Add(wakeLoading.Result[0]);
                wakeLoading.Result[0].TryGetComponent(out wl.lineRenderer);
                wakeLoading.Result[0].hideFlags = HideFlags.HideAndDontSave;
